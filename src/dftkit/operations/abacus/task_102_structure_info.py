...
def _read_structure(task_input: StructureInfoInput):
    normalized_format = task_input.format.strip().lower()
    if normalized_format in {"auto", "abacus", "abacus/stru", "stru"}:
        if normalized_format != "auto" or task_input.input.name.upper() == "STRU":
            system = dpdata.System()
            system.from_abacus_stru(task_input.input)
            atoms = system.to_ase_structure()
    ...
    return read(
        task_input.input,
        format=None if normalized_format == "auto" else normalized_format,
    )


def run_structure_info(task_input: StructureInfoInput) -> dict[str, Any]:
    atoms = _read_structure(task_input)
