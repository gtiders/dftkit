...
from dftkit.schemas.abacus.file_conversion import PoscarToStruSwapACInput


def _swap_a_c_axes(atoms) -> None:
    cell = atoms.get_cell()[:]
    positions = atoms.get_positions()
    new_positions = positions[:, [2, 1, 0]]
    new_cell = cell[:, [2, 1, 0]][[2, 1, 0], :]
...
def run_poscar_to_stru_swap_ac_convert(
    task_input: PoscarToStruSwapACInput,
) -> dict[str, Any]:
    atoms = ase.io.read(task_input.input)
    _swap_a_c_axes(atoms)

    basis_config = (
        _load_basis_config(task_input.basis_json) if task_input.basis_json is not None else {}
    )
    ...
    try:
        ase.io.write(tmp_poscar, atoms, format="vasp")
        system = dpdata.System()
        system.from_poscar(tmp_poscar)
        kwargs: dict[str, Any] = {}
    ...
    return {
        "task": "poscar-to-stru-swap-ac-convert",
